...
$(document).ready(function() {
  ...
  setInterval(function(){
    if(game.state.action) $('#action').text(game.state.action.name);
    else $('#action').text('_______');
  },1000);
  return;
...
function draw_map() {
  ...
  for (x=0; x < game.mapsize_x; x++) {
    for (y=0; y < game.mapsize_y; y++) {
      ...
      hex_tiles +='<div id="'+ x + '_' + y +'" class="hex '+game.mapArray[x][y].join(' ')+'" style="position:absolute;z-index:'+z_index+';left:' + hex_x + 'px;top:' + hex_y + 'px;">';
      ...
      hex_tiles +='</div>';
      hex_tiles += '<div class="hex_l"><div class="hex_r"><div class="hex_inner" style="background-position:'+hexer.getRandomInt(0,101)+'% '+hexer.getRandomInt(0,101)+'%;"></div></div></div>';
      hex_tiles += '</div>';
    }
    z_index--;
  }
